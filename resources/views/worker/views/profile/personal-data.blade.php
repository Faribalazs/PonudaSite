<x-worker-profile-layout>
    ...
    <div class="flex profile-title">
        <p class="{{ Auth::guard('worker')->user()->hasRole('super_worker') ? '' : 'mb-4'}} text-3xl font-bold">Podaci firme</p>
    </div>
    ...
    @if (!empty($company_data))
        <div class="flex mt-3 flex-col">
            ...
            <div class="flex gap-1 flex-col mb-3">
                ...
                <img src="{{ url('storage/'. $company_data->logo) }}"
                alt="{{ $company_data->company_name }}" style="width:500px; height: 240px; object-fit: cover;">
            </div>
        </div>
    @else
        <div class="flex mt-3 flex-col">
            <form method="POST" action="{{ route('worker.personal.data.save') }}" class="flex flex-col"
                ...
                <input class="input-style {{$errors->has('country') ? 'border-error mb-1' : 'mb-3'}}"
                    name="country"
                    value="{{ old('country') }}"
                    maxlength="20"
                    type="text"
                    required/>
                <p class="{{$errors->has('country') ? 'flex text-red mt-1 pl-1' : 'hidden'}}">{{$errors->first('country')}}</p>
                ...
                <input class="input-style {{$errors->has('address') ? 'border-error mb-1' : 'mb-3'}}"
                    name="address"
                    value="{{ old('address') }}"
                    maxlength="50"
                    type="text"
                ...
                <div class="flex lg:flex-row flex-col">
                    ...
                    <div class="flex w-full lg:w-1/2 flex-col lg:pl-2 pl-0">
                        ...
                        <input class="input-style {{$errors->has('maticni_broj') ? 'border-error mb-1' : 'mb-3'}}"
                            ...
                            type="text"
                            required/>
                        <p class="{{$errors->has('maticni_broj') ? 'flex text-red mt-1 pl-1' : 'hidden'}}">{{$errors->first('maticni_broj')}}</p>
                ...
                <div class="flex lg:flex-row flex-col">
                    <div class="flex w-full lg:w-1/2 flex-col lg:pr-2 pr-0">
                        ...
                        <input class="input-style {{$errors->has('tekuci_racun') ? 'border-error mb-1' : 'mb-3'}}"
                            name="tekuci_racun"
                            value="{{ old('tekuci_racun') }}"
                            maxlength="30"
                            type="text"
                            required/>
                        <p class="{{$errors->has('tekuci_racun') ? 'flex text-red mt-1 pl-1' : 'hidden'}}">{{$errors->first('tekuci_racun')}}</p>
                    ...
                    <div class="flex w-full lg:w-1/2 flex-col lg:pl-2 pl-0">
                        ...
                        <input class="input-style {{$errors->has('bank_name') ? 'border-error mb-1' : 'mb-3'}}"
                            ...
                            type="text"
                            required/>
                        <p class="{{$errors->has('bank_name') ? 'flex text-red mt-1 pl-1' : 'hidden'}}">{{$errors->first('bank_name')}}</p>
                    </div>
                </div>
                ...
                <input class="input-style {{$errors->has('bank_account') ? 'border-error mb-1' : 'mb-3'}}"
                    ...
                    maxlength="30"
                    type="text" />
                <p class="{{$errors->has('bank_account') ? 'flex text-red mt-1 pl-1' : 'hidden'}}">{{$errors->first('bank_account')}}</p>
                ...
                <div class="flex flex-col sm:flex-row">
                    ...
                    <input id="uploadFile" class="text-center sm:text-left sm:pl-3 pl-0 sm:mt-0 mt-2 max-w-full" placeholder="Niste izabrali sliku" disabled="disabled" />
                </div>
                <p class="{{$errors->has('logo') ? 'flex text-red mt-2 pl-1' : 'hidden'}}">{{$errors->first('logo')}}</p>
                <button type="submit" class="finish-btn sm:mt-20 mt-12 text-xl">Sacuvaj podatke</button>
            </form>
        </div>
        <script>
